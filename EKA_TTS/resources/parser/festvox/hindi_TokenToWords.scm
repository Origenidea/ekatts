...
;;commented by pranaw 24-01-2016  
 ; ("0" "शून्य")
  ;  ("1" "एक")
   ;   ("2" "दो")
    ;("3" "तीन")
    ...
    ;("8" "आठ")
    ;("9" "नौ")
;; added by pranaw for navbharat times
 ("0" "जीरो")
    ("1" "वन")
      ("2" "टू")
    ("3" "थ्री")
...
(defvar hindi_supplementary_char_list
  '("£" "€" "¥" "©" "®" "¢" "।" "॥" "‑" "č" "a" "ö" "a" "ā" "o" "e" "ee" "u" "uu" "ru" "λ" "Δ" "@" "å" "k‑s‑h" "k-h" "k" "g" "g-h" "c" "c-h" "j" "z" "t" "t-h" "d" "d-h" "л" "ŧ" "ŧ-h" "đ" "đ-h" "n" "p" "ø" "s‑ŧ‑h" "p-h" "b" "b-h" "m" "y" "r" "l" "φ""w" "s‑h" "x" "xø" "s" "h" "k-k" "g" "j-j" "z-z" "dø" "d-hø" "f" "л-h" "n-h" "m-h" "r-h" "l-h" "w-h" "ŋ"))
...
(define (hindi_token_to_words_ token name)
  ...
  (cond
   ...
   ((string-matches name (string-append "रू\\.[" number_regex))
        (if (hindi_list_lookup (item.feat token "n.name") hindi_denominations_list)
	  (hindi_token_to_words_ token (string-after name "रू."))
	  (list (hindi_token_to_words_ token (string-after name "रू.")) "रूपए")))
    ((and (string-matches (item.feat token "p.name") (string-append "रू\\.[" number_regex))
	          (hindi_list_lookup name hindi_denominations_list) ) (list name "रूपए"))
   ;;cents
  ...
  ;; Numbers with decimal point
    ((string-matches name "[0-9]*\\.[0-9]+") 
        (list (hindi_number_to_words (string-before name ".")) '("दसमलव") 
